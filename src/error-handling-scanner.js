  ...
  async analyzeLabelsInstructions(page, violations) {
    ...
    const labelsAnalysis = await page.evaluate(() => {
      ...
      function statesRequirement(text) {
        ...
        return /\brequired\b|\bmandatory\b/.test(t) ||
        ...
        if (needsFormatInstructions.includes(fieldType)) {
          const hasFormatInstruction = field.hasAttribute('aria-describedby') ||
                                     ...
                                     (field.labels && Array.from(field.labels).some(label =>
                                       statesFormatRequirement(label.textContent)
                                     ));
      ...
      forms.forEach(form => {
        ...
        const surroundingText = (surroundings ? surroundings.textContent : form.textContent).toLowerCase();
        ...
        if (isComplexForm && !hasFormInstructions && isVisible && hasUninstructedFields) {
          issues.push({
            ...
            element: 'form',
            description: 'Complex form lacks overall instructions and contains fields without their own labels or instructions',
            severity: 'warning'
          });
        }
      });
  ...
  async analyzeErrorPrevention(page, violations) {
    ...
    const preventionAnalysis = await page.evaluate(() => {
      ...
      const IMPORTANT_DATA_TERMS = [
        'payment', 'purchase', 'order', 'invoice', 'checkout', 'contract', 'billing',
        'zahlung', 'bezahl', 'überweis', 'ueberweis', 'bestellung', 'bestellen',
        'kauf', 'rechnung', 'vertrag', 'kredit', 'darlehen', 'iban', 'kontonummer',
        'kreditkarte', 'lastschrift', 'betrag', 'honorar', 'gebühr'
      ];
      ...
      const IMPORTANT_FIELD_SELECTOR = [
        'input[name*="card"]', 'input[name*="payment"]', 'input[name*="iban"]',
        'input[name*="konto"]', 'input[name*="karte"]', 'input[name*="betrag"]',
        'input[id*="card"]', 'input[id*="iban"]', 'input[id*="karte"]', 'input[id*="betrag"]',
        'input[autocomplete*="cc-"]'
      ].join(', ');
      const CONFIRM_TERMS = ['confirm', 'verify', 'bestätig', 'bestaetig', 'freigeb'];
      const REVIEW_TERMS = ['review', 'edit', 'prüfen', 'pruefen', 'überprüf', 'ueberpruef', 'kontrollier', 'korrigier'];

      const forms = document.querySelectorAll('form');

      forms.forEach(form => {
        const formText = form.textContent.toLowerCase();
        const hasImportantData = form.querySelector(IMPORTANT_FIELD_SELECTOR) ||
        ...
        if (hasImportantData) {
          ...
          const hasReviewStep = REVIEW_TERMS.some(term => formText.includes(term)) ||
