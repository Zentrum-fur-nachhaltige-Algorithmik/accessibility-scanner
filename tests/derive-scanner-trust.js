 ...
 *   docs/sprints/p2-quality/raw/harness-exclusive.json
 *   docs/sprints/p2-quality/raw/harness-concurrent.json
 *   docs/sprints/p2-quality/raw/harness-realworld.json
 *   docs/sprints/p2-quality/raw/good-file-noise-breakdown.json
 *
 * Usage:
 *   node tests/derive-scanner-trust.js            # rewrite src/scanner-trust.json
 *   node tests/derive-scanner-trust.js --check    # fail if the file is stale
 */
...
function derive() {
  ...
  for (const { id, kind } of scanners) {
    ...
    if (kind === 'llm') {
      trust[id] = {
        ...
        kind,
        reason:
          'LLM scanner — measured by test-sites/test-llm-scanners.js (violation-level ' +
  ...
  return {
    ...
    note:
      'GENERATED by tests/derive-scanner-trust.js from the checked-in battery results. ' +
...
function main() {
  ...
  if (check) {
    if (!fs.existsSync(OUT)) {
      console.error('scanner-trust.json missing — run: node tests/derive-scanner-trust.js');
      process.exit(1);
    }
    ...
    if (!same) {
      console.error('scanner-trust.json is stale — re-run: node tests/derive-scanner-trust.js');
      process.exit(1);
    }
