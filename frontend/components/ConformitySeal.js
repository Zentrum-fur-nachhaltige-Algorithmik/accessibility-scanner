/**
 * Conformity seal of the audit office.
 *
 * Built from the same braille motif as the society's logo (the Nemeth braille
 * "equals" sign — equality doubles as conformity): concentric rings, a milled
 * tick ring, and the braille cells in the centre.
 *
 * The seal is pure geometry and carries no text node. Curved SVG lettering
 * would be prettier, but axe-core cannot determine a background colour for
 * any <text>/<textPath> inside a drawing and reports "incomplete" for it —
 * verified against a lone <textPath> with no siblings and an opaque parent.
 * The standing legend and the verdict are therefore set as real HTML text
 * beside the seal (see .pb-seal-legend / .pb-verdict-label), where they are
 * selectable, zoomable, translatable and contrast-checkable.
 *
 * Two sizes share the drawing: a small decorative mark in the letterhead and
 * the large stamp next to the score.
 */

// Braille cell geometry, taken from the logo on the start page: cell 1 carries
// dots 4 and 6, cell 2 carries dots 1 and 3.
...
export default function ConformitySeal({ size = 104, label = null }) {
  ...
  // the office name and would only repeat it.
