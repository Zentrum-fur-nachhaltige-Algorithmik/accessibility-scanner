  ...
  async testRouteChanges(page, scanDir, options) {
    ...
    try {
      ...
      const navLinks = await page.$$(
        'a[href*="#"], a[href^="/"], .nav-link, .route-link, [data-route]'
      );
      ...
      for (let i = 0; i < Math.min(navLinks.length, 5); i++) {
        ...
        try {
          ...
          const stillSameDocument = await page
            ...
            loadingIndicators.forEach((indicator, idx) => {
              if (
                ...
                violations.push({
                  ...
                  severity: 'moderate',
                  element:
                    indicator.tagName.toLowerCase() +
                    (indicator.className ? `.${indicator.className.split(' ')[0]}` : ''),
                  description:
                    'Loading indicator during route change lacks accessibility attributes',
                  details: {
                    linkIndex: linkIndex,
                    indicatorIndex: idx,
  ...
  async testDynamicContentUpdates(page, scanDir, options) {
    return await page.evaluate(() => {
      ...
      const searchResults = document.querySelectorAll(
        '.search-results, .results-container, .filter-results, ' + '[data-results], .search-output'
      );

      searchResults.forEach((results, index) => {
        if (!results.hasAttribute('aria-live') && !results.closest('[aria-live]')) {
          violations.push({
  ...
  async testLoadingStates(page, scanDir, options) {
    return await page.evaluate(() => {
      ...
      loadingElements.forEach((loading, index) => {
        ...
        if (!loading.hasAttribute('aria-live') && !loading.closest('[aria-live]')) {
          violations.push({
          ...
          if (submitButton) {
            ...
            const formViolations = await page.evaluate((formIndex) => {
              ...
              errorMessages.forEach((error, errorIndex) => {
                if (
                  ...
                  violations.push({
                    ...
                    severity: 'serious',
                    element:
                      error.tagName.toLowerCase() +
                      (error.className ? `.${error.className.split(' ')[0]}` : ''),
              ...
              successMessages.forEach((success, successIndex) => {
                if (
                  !success.hasAttribute('aria-live') &&
                  ...
                  violations.push({
                    ...
                    severity: 'moderate',
                    element:
                      success.tagName.toLowerCase() +
                      (success.className ? `.${success.className.split(' ')[0]}` : ''),
  ...
  async testModalDialogs(page, scanDir, options) {
    ...
    try {
      ...
      const modalTriggers = await page.$$(
        'button[data-modal], .modal-trigger, [data-toggle="modal"], .open-modal'
      );
      ...
      for (let i = 0; i < Math.min(modalTriggers.length, 3); i++) {
        ...
        try {
          ...
          const closeButton = await page.$(
            '.modal .close, .modal [aria-label*="close"], .modal .modal-close'
          );
          if (closeButton) {
  ...
  async testInfiniteScroll(page, scanDir, options) {
    return await page.evaluate(() => {
      ...
      const scrollContainers = document.querySelectorAll(
        '.infinite-scroll, [data-infinite], .lazy-load, ' + '.scroll-container, [class*="infinite"]'
      );

      scrollContainers.forEach((container, index) => {
        // Check for proper loading announcements
        if (!container.querySelector('[aria-live]') && !container.hasAttribute('aria-live')) {
          violations.push({
  ...
  async testSearchFilters(page, scanDir, options) {
    ...
    try {
      const searchInputs = await page.$$(
        'input[type="search"], .search-input, [placeholder*="search"]'
      );

      for (let i = 0; i < Math.min(searchInputs.length, 2); i++) {
        // Limit to 2 search inputs
        ...
        try {
          ...
          const searchViolations = await page.evaluate((searchIndex) => {
            ...
            const resultsContainers = document.querySelectorAll(
              '.search-results, .results, .filter-results, ' + '[data-results], .search-output'
            );
            ...
            resultsContainers.forEach((results, index) => {
              ...
              if (!results.hasAttribute('aria-live') && !results.closest('[aria-live]')) {
                violations.push({
                  ...
                  severity: 'serious',
                  element:
                    results.tagName.toLowerCase() +
                    (results.className ? `.${results.className.split(' ')[0]}` : ''),
  ...
  generateDynamicSpaRecommendations(violations) {
    ...
    if (issueTypes.some((type) => type.includes('live-region'))) {
      recommendations.push({
        ...
        implementation:
          'Add aria-live="polite" for updates, aria-live="assertive" for urgent alerts',
      });
    }
    ...
    if (issueTypes.some((type) => type.includes('modal-dialog'))) {
      recommendations.push({
        ...
        implementation:
          'Use role="dialog", aria-modal="true", proper labeling, and focus management',
      });
    }
