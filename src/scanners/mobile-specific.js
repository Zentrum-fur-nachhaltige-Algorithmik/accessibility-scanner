...
class MobileSpecificScanner extends BaseScanner {
  ...
  async test400PercentZoom(page, deviceName, viewport) {
    return await page.evaluate(
      (device, viewportInfo, renderedSrc, clipSrc) => {
        eval(renderedSrc);
        eval(clipSrc);
        const violations = [];
        ...
        function isSrOnly(el) {
          ...
          if (
            typeof cls === 'string' &&
            (/\bsr-only\b/.test(cls) || /\bvisually-hidden\b/.test(cls))
          )
            return true;
            ...
            if (typeof sel === 'string' && sel && !sel.includes('::')) {
              ...
              if (declaredWidth && declaredWidth.endsWith('px')) {
                try {
                  ...
                  /* selector not supported by querySelectorAll */
                }
              }
            }
        ...
        for (let i = 0; i < fixedWidthElements.length; i++) {
          ...
          const isSelfFlexGrid =
            computedStyle.display === 'flex' ||
            computedStyle.display === 'grid' ||
            computedStyle.display === 'inline-flex' ||
            computedStyle.display === 'inline-grid';
          const skipWidthCheck =
            !hasFixedCssWidth || isResponsive || isFlexGridChild || isSelfFlexGrid;
          ...
          if (!skipWidthCheck && width && width.endsWith('px')) {
            ...
            if (widthValue > viewportInfo.width * 0.5) {
              violations.push({
                ...
                recommendation:
                  'Use relative units (%, em, rem) or max-width: 100% for mobile compatibility',
              });
            }
          }
        ...
        for (const clip of window.__findClippedText({ minChars: 3 })) {
          violations.push({
            ...
            recommendation:
              'Use overflow: auto/visible or let the container reflow instead of clipping',
          });
        }
  ...
  deduplicateAcrossDevices(violations) {
    ...
    for (const v of violations) {
      ...
      if (map.has(key)) {
        ...
        if (device && !existing.affectedDevices.includes(device))
          existing.affectedDevices.push(device);
        continue;
      }
  ...
  async testHorizontalScrolling(page, deviceName, viewport) {
    return await page.evaluate(
      (device, viewportInfo) => {
        ...
        const allElements =
          bodyScrollWidth > windowWidth + 1 ? document.querySelectorAll('body *') : [];
        ...
        for (let i = 0; i < allElements.length; i++) {
          ...
          if (rect.right > windowWidth + 10 && !clippedByAncestor(element)) {
            // 10px tolerance
            const computedStyle = window.getComputedStyle(element);
            ...
            if (!scrollableAncestor) {
              violations.push({
                ...
                details: {
                  ...
                  viewportWidth: viewportInfo.width,
                  columnCount:
                    table.querySelectorAll('th, td').length / table.querySelectorAll('tr').length,
                },
                wcagCriteria: '1.4.10',
                impact: 'Table may be difficult to read on mobile devices',
                recommendation:
                  'Wrap table in responsive container or use mobile-friendly table design',
              });
            }
          }
        }
  ...
  generateMobileRecommendations(violations) {
    ...
    if (issueTypes.some((type) => type.includes('viewport'))) {
      recommendations.push({
        ...
        implementation:
          'Use <meta name="viewport" content="width=device-width, initial-scale=1.0"> without user-scalable=no',
      });
    }
    ...
    if (issueTypes.some((type) => type.includes('400-percent-zoom'))) {
      recommendations.push({
        ...
        implementation:
          'Use relative units, flexible layouts, and test at 400% zoom on mobile devices',
      });
    }
    ...
    if (issueTypes.some((type) => type.includes('horizontal-scroll'))) {
      recommendations.push({
        ...
        implementation:
          'Use CSS media queries, flexible grids, and max-width: 100% for responsive design',
      });
    }
