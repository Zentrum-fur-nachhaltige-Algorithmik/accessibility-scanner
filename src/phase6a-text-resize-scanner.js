    ...
    async analyzeTextResizeCompliance(page, viewport, scanDir) {
        ...
        const fixedElements = await page.evaluate(() => {
            ...
            for (const el of allElements) {
                ...
                if (typeof cls === 'string' && (/\bsr-only\b/.test(cls) || /\bvisually-hidden\b/.test(cls))) continue;

                // Skip elements using responsive layout
                const maxW = styles.maxWidth;
                const isResponsiveMaxWidth = maxW && (maxW.endsWith('%') || maxW.endsWith('vw') || maxW === '100%');
                ...
                const parentWidth = el.parentElement ? el.parentElement.getBoundingClientRect().width : rect.width;
                // Element fills parent = natural flow, not fixed width
                const fillsParent = Math.abs(rect.width - parentWidth) < 2;
                const isBlockDisplay = styles.display === 'block' || styles.display === 'flex' || styles.display === 'grid' ||
                    styles.display === 'table' || styles.display === 'list-item' ||
                ...
                const isFlexGridChild = parentStyles && (parentStyles.display === 'flex' || parentStyles.display === 'grid' ||
                    parentStyles.display === 'inline-flex' || parentStyles.display === 'inline-grid');
                ...
                const maxWidthTooSmall = styles.maxWidth && styles.maxWidth.endsWith('px') && parseInt(styles.maxWidth) < 200;
