      ...
      selectors.forEach(selector => {
        document.querySelectorAll(selector).forEach((element, index) => {
          ...
          if (rect.width > 0 && rect.height > 0) {
            ...
            if (!element.id && !element.className) elementSelector += `:nth-of-type(${index + 1})`;
    ...
    const interactiveElements = await page.evaluate(() => {
      ...
      selectors.forEach(selector => {
        document.querySelectorAll(selector).forEach((element, index) => {
          const rect = element.getBoundingClientRect();
          if (rect.width > 0 && rect.height > 0) {
            // Generate more reliable selector
            let elementSelector = element.tagName.toLowerCase();
            if (element.id) {
            ...
            if (!element.id && !element.className) {
              ...
              if (parent) {
                ...
                elementSelector = `${parentSelector} > ${elementSelector}:nth-child(${elementIndex + 1})`;
              } else {
