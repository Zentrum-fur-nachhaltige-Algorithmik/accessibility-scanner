  ...
  async performMediaAnalysis(page, scanDir, options) {
    ...
    const mediaAnalysis = await page.evaluate(() => {
      ...
      videos.forEach(video => {
        ...
        const isHidden = video.offsetParent === null;
        ...
        if (tracks.length === 0) {
          ...
          allIssues.push({
            type: 'video-caption',
            element: selector,
            description: 'Video lacks captions or subtitles',
            severity: 'serious',
            suggestion: 'Add <track> elements with kind="captions" or kind="subtitles"'
          });
        }
        
        const audioDescTracks = video.querySelectorAll('track[kind="descriptions"]');
        if (audioDescTracks.length === 0) {
          allIssues.push({
            type: 'video-description',
            element: selector,
            description: 'Video may lack audio descriptions for visual content',
            severity: 'moderate',
            suggestion: 'Consider adding audio descriptions via <track kind="descriptions"> for videos with important visual information'
          });
        }
      ...
      audioElements.forEach(audio => {
        ...
        const isHidden = audio.offsetParent === null;
        ...
        const hasTranscriptLink = parent && (
    ...
    mediaAnalysis.allIssues.forEach(issue => {
      let criterion = "9.1.1.1"; // Default criterion
      if (issue.type.includes('video')) criterion = "9.1.2.2";
      if (issue.type.includes('audio')) criterion = "9.1.2.1";
