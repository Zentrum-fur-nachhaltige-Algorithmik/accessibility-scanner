  ...
  async scan(page, options = {}) {
    const colorDependencyResults = await page.evaluate(() => {
      ...
      function hasNonColorIndicators(element) {
        ...
        if (
          textDecoration &&
          (textDecoration.includes('underline') || textDecoration.includes('overline'))
        )
          return true;
        ...
        const isInNav = element.closest(
          'nav, .nav, .navigation, .menu, header, footer, .header, .footer'
        );
      ...
      links.forEach((link) => {
        ...
        const isInMainContent = !link.closest(
          'nav, .nav, .navigation, .menu, header, footer, .header, .footer, .sidebar, aside'
        );
        ...
        if (hasColorDifference && isInMainContent && !hasNonColorIndicators(link)) {
          ...
          const hasContext =
            link.closest('p, article, .content, main') &&
          ...
          if (hasContext) {
            ...
            violations.push({
              ...
              description:
                'Link in content area is differentiated from surrounding text only by color',
              suggestion:
                'Add underline, bold text, or other visual indicator to distinguish links',
            });
          }
        }
      });
      ...
      formElements.forEach((formElement) => {
        const fieldContainer =
          formElement.closest('.form-group, .field, .input-group') || formElement.parentElement;

        // Look for error indicators
        const errorElements = fieldContainer.querySelectorAll(
          '.error, .invalid, .warning, [class*="error"], [class*="invalid"]'
        );
        errorElements.forEach((errorElement) => {
          ...
          const isRedish =
            errorColor.includes('rgb(255, 0, 0)') ||
            errorColor.includes('red') ||
            errorColor.includes('rgb(220, 53, 69)') ||
            errorColor.includes('#ff0000');

          // Check if error uses only color without adequate text or icons
          if (
            isRedish &&
            text.toLowerCase() === 'error message' &&
            !hasNonColorIndicators(errorElement)
          ) {
            formErrorsColorOnly++;
            colorOnlyElements++;
            violations.push({
        ...
        const requiredIndicators = fieldContainer.querySelectorAll(
      ...
      const statusElements = document.querySelectorAll(
        '.status, .alert, .message, .notification, [class*="status"], [class*="alert"], [class*="success"], [class*="warning"], [class*="info"]'
      );
      statusElements.forEach((statusElement) => {
        const hasColorStyling =
          getComputedColor(statusElement, 'background-color') !== 'rgba(0, 0, 0, 0)' ||
          getComputedColor(statusElement, 'color') !==
            getComputedColor(statusElement.parentElement, 'color');

        if (
          hasColorStyling &&
          !hasTextIndicators(statusElement) &&
          !hasNonColorIndicators(statusElement)
        ) {
          colorOnlyElements++;
          violations.push({
      ...
      charts.forEach((chart) => {
        ...
        const hasLegend =
          chart.querySelector('.legend, [class*="legend"]') ||
