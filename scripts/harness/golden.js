...
const FORBIDDEN = {
  ...
  'interaction-blocked':                  { fp: 'FP-2',  criteria: ['1.4.4'], max: 0, real: ['warmth', 'lumen', 'spectrum'], realMax: 1 },
  ...
  'focus-obscured-by-sticky-element':     { fp: 'FP-9',  criteria: ['2.4.11'], max: 0, real: ['lumen', 'warmth'], realMax: 1 },
  'focus-obscured-by-fixed-element':      { fp: 'FP-9',  criteria: ['2.4.11'], max: 0, real: ['evergreen', 'warmth', 'lumen', 'spectrum'], realMax: 1 },
 ...
 */
const EXPECTED = [
  { rule: 'insufficient-focus-indicator-contrast', why: 'footer focus ring 1.41:1 (1.4.11)', themes: ['evergreen'] },
  ...
  { rule: 'text-spacing-failure', why: 'stat label clipped under 1.4.12 spacing (1.4.12)', themes: ['evergreen', 'clinic'] },
  ...
  { rule: 'focus-obscured-by-sticky-element', why: 'skip link hidden behind header z-index (2.4.11)', themes: ['lumen', 'warmth'] },
  { rule: 'focus-obscured-by-fixed-element', why: 'footer links under fixed sticky-cta (2.4.11)', themes: ['evergreen'] },
  { rule: 'interaction-blocked', why: 'sticky-cta covers footer links at <=640px (1.4.10)', themes: ['warmth', 'lumen', 'spectrum'] },
];
...
const MIME = {
  ...
  '.woff2': 'font/woff2', '.woff': 'font/woff', '.ico': 'image/x-icon', '.svg': 'image/svg+xml',
  '.png': 'image/png', '.jpg': 'image/jpeg', '.webp': 'image/webp', '.txt': 'text/plain', '.json': 'application/json',
};
...
function serve(rootDir) {
  const server = http.createServer((req, res) => {
    ...
    if (!p.startsWith(rootDir)) { res.writeHead(403); return res.end(); }
...
async function scanPage(url, scannerIds, scanOptions) {
  ...
  const timeout = new Promise((_, rej) => {
    timer = setTimeout(() => rej(new Error(`WALL-CLOCK TIMEOUT after ${PER_PAGE_TIMEOUT_MS}ms`)), PER_PAGE_TIMEOUT_MS);
  });
  try {
    return await Promise.race([pipeline.scan(url, { ...scanOptions, scannerIds: ids, timeout: 60000 }), timeout]);
  } finally {
    clearTimeout(timer);
...
async function main() {
  ...
  for (const theme of args.themes) {
    ...
    if (!fs.existsSync(rootDir)) { failures.push(`corpus missing: ${rootDir}`); continue; }
    ...
    try {
      for (const route of args.routes) {
        ...
        const crashed = Object.entries(result.scanners || {}).filter(([, s]) => s.error).map(([id, s]) => `${id}: ${s.error}`);
        ...
        if (critical.length) pageFail.push(`${critical.length} critical finding(s): ${[...new Set(critical.map(ruleOf))].join(', ')}`);
        ...
        // went blind.
        for (const e of EXPECTED) for (const v of violations) if (ruleOf(v).includes(e.rule)) seenExpected[theme].add(e.rule);

        const total = Object.values(counts).reduce((a, b) => a + b, 0);
        console.log(`${pageFail.length ? 'FAIL' : 'ok  '} score=${String(result.accessibilityScore).padStart(3)} findings=${String(total).padStart(4)} critical=${critical.length}`);
        for (const f of pageFail) { console.log(`      - ${f}`); failures.push(`${label}: ${f}`); }
        report.pages.push({ theme, route, score: result.accessibilityScore, findings: total, critical: critical.length, counts, failed: pageFail });
      }
    } finally {
      server.close();
    }
