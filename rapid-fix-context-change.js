...
async function rapidFixContextChange() {
  const scanner = new ColorContrastScanner();
  const fileUrl = `file://${__dirname}/test-sites/good-context-change.html`;
  
  console.log('🔍 RAPID FIX: good-context-change.html');
...
rapidFixContextChange().catch(console.error);